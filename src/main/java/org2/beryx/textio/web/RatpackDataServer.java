...
//                        chain = chain.insert(handler);
    ...
    protected void sendResponseData(Context ctx, ResponseData r) {
        // added brito
        // get the session id
        String sessionId = ctx.getRequest().oneCookie("JSESSIONID");
        // when there are cookies, add them up
        if(ConsoleWeb.hasSessionId(sessionId)){
            for(CookieInternal cookie : ConsoleWeb.dataList.get(sessionId).getCookies()){
                ctx.getResponse().cookie(cookie.getKey(), cookie.getValue());
            }
        }
        // added brito
        //ctx.getResponse().cookie("time","" + System.currentTimeMillis());
        ctx.getResponse()
                .status(r.status)
